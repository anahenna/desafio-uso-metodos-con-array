...
traumatologia.push({ hora: '9:30', especialista: 'MARIA SOLAR', paciente: 'RAMIRO ANDRADE ', rut: '12221451-K', prevision: 'FONASA' })
...
document.body.innerHTML += '<h2>Horas de consulta médicas Dental:</h2>';
dental.forEach(function (item) {
    document.body.innerHTML += "<p>" + item.hora + " - " + item.especialista + " - " + item.paciente + " - " + item.rut + " - " + item.prevision
})

document.body.innerHTML += '<h2>Listado de todos los pacientes atendidos el día de hoy:</h2>';
radiologia.forEach(function (item) {
    document.body.innerHTML += "<p>" + item.paciente
})

traumatologia.forEach(function (item) {
    document.body.innerHTML += "<p>" + item.paciente
})
...
const pacientesIsapre = dental
    .filter(paciente => paciente.prevision === 'ISAPRE')
    .map(({ paciente, prevision }) => `${paciente} - ${prevision}`);
console.log(pacientesIsapre);

const pacientesFonasa = traumatologia
    .filter(paciente => paciente.prevision === 'FONASA')
    .map(({ paciente, prevision }) => `${paciente} - ${prevision}`);
console.log(pacientesFonasa);


//Agregar código para el desafio 2 aquí
document.body.innerHTML += '<h2>Cantidad de atenciones por especialidad:</h2>';
document.write(`<p>Cantidad de atenciones para Radiología: ${radiologia.length}</p>`);
document.write(`<p>Cantidad de atenciones para Traumatología: ${traumatologia.length}</p>`);
document.write(`<p>Cantidad de atenciones para Dental: ${dental.length}</p>`);

document.body.innerHTML += '<h2>Primera y última atencion por especialidad:</h2>';
